...
import { Migrator__factory } from "../../typechain";
...
task("deploy-migrator")
  ...
  .setAction(async (taskArgs, { ethers, run }) => {
    ...
    const migrator = await new Migrator__factory(signers[0]).deploy(
      taskArgs.source,
    ...
    if (taskArgs.verify) {
      ...
      await run("verify:verify", {
        ...
        constructorArguments: [taskArgs.source, taskArgs.destination, taskArgs.rate],
      });
    }
